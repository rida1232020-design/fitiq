...
export default function ChatRoom({ conversation, currentUser, profile, onBack }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:C.bg, fontFamily:'Cairo,sans-serif', direction:'rtl' }}>
      ...
      <header style={{ background:'#fff', borderBottom:`1px solid ${C.border}`, padding:'12px 16px', display:'flex', alignItems:'center', gap:12, boxShadow:C.shadow, position:'sticky', top:0, zIndex:10 }}>
        <button onClick={onBack} style={{ background:C.bgMuted, border:'none', borderRadius:10, width:36, height:36, fontSize:18, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>←</button>
        <Avatar emoji={otherProfile?.avatar_emoji || '💪'} size={42} style={{ background:`linear-gradient(135deg,${C.accent},#0080FF)`, boxShadow:'none' }}/>
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:800, fontSize:15, color:C.text }}>{otherProfile?.full_name || 'محادثة'}</div>
          <div style={{ color:C.success, fontSize:11, fontWeight:700 }}>● متصل</div>
        </div>
      </header>
          ...
          : messages.length === 0
            ? (
              <div style={{ textAlign:'center', marginTop:60 }}>
                ...
                <div style={{ color:C.textMuted, fontSize:14, marginTop:10 }}>ابدأ المحادثة</div>
              </div>
            )
            : messages.map((m, i) => {
              ...
              return (
                <div key={m.id || i}>
                  ...
                  <div style={{ display:'flex', justifyContent: isMine ? 'flex-start' : 'flex-end', animation:'fadeIn 0.25s ease' }}>
                    <div style={{ maxWidth:'78%', display:'flex', flexDirection:'column', alignItems: isMine ? 'flex-start' : 'flex-end', gap:2 }}>
                      <div style={{
                        ...
                        border: isMine ? `1.5px solid ${C.border}` : 'none',
                      }}>
                        {m.content}
                      </div>
      ...
      <div style={{ background:'#fff', borderTop:`1px solid ${C.border}`, padding:'12px 14px', display:'flex', gap:8, alignItems:'flex-end' }}>
        <textarea
          ...
          style={{
            ...
            direction:'rtl', outline:'none', resize:'none', fontFamily:'Cairo,sans-serif',
            maxHeight:100, lineHeight:1.5, overflowY:'auto',
          }}
        />
        <button onClick={send} disabled={sending || !input.trim()} style={{
          ...
          boxShadow: input.trim() ? `0 4px 14px rgba(255,77,0,0.3)` : 'none', transition:'all 0.2s',
          flexShrink:0,
        }}>
